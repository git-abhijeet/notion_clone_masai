...
function DocumentsIdPage({ params }: DocumentsIdPageProps) {
  ...
  const handleGenerateTags = async () => {
    if (document?.content) {
      const tags = await generateTags(document.content, document.title || "Untitled");
      if (tags?.length > 0) {
        await update({ 
  ...
  return (
    <div className="pb-40">
      ...
      <div className="md:max-w-3xl lg:max-w-4xl mx-auto">
        ...
        {((document.aiGeneratedTags && document.aiGeneratedTags.length > 0) || isGeneratingTags) && (
          <div className="px-[54px] py-4">
            <div className="flex items-center gap-2 mb-2">
              <h3 className="text-sm font-medium text-muted-foreground">AI Generated Tags:</h3>
              {isGeneratingTags && (
